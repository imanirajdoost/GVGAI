...
public class SingleMCTSPlayer {
    ...
    public int continueRun(ElapsedCpuTimer elapsedTimer, StateObservation a_gameState) {

        if (m_root == null) {
            System.out.println("Root is null, creating a new root");
            m_root = new SingleTreeNode(m_rnd, num_actions, actions);
            m_root.rootState = a_gameState;
        }

        //Continue the search within the available time.
        m_root.mctsSearch(elapsedTimer);

        //Determine the best action to take and save it.
