...
public class Atom2SetReduced extends AtomSet {
  ...
  public Atom2SetReduced(BfsNode node) {
    ...
    ArrayList<Long> missingObsIdAtoms = getMissingObsIdAtoms(node);
    for (long moiAtom : missingObsIdAtoms) {
    ...
    ArrayList<Long> obsTypeAtoms = getObsTypeAtoms(node);
    for (long otAtom : obsTypeAtoms) {
      long right = (long)(otAtom * NUM_ATOM_TYPES + 2);
      this.atoms.add(left | right);
    }

    // {avatar, resource} (resource has atom type 3)
    ArrayList<Long> resourceAtoms = getResourceAtoms(node);
